  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const baseUrl = BASE_URL || `${url.protocol}//${url.host}`;
        ...
        const [cats, prods, posts] = (await Promise.all([
          ...
          prodsCol
            .find({ is_published: true }, { projection: { slug: 1, category_id: 1 } })
            .toArray(),
        ...
        for (const p of prods) {
          if (p.category_id && p.slug) {
            const catRow = cats.find(
              (c: any) => c._id?.toString() === p.category_id.toString(),
            ) as any;
        ...
        const xml = [
          ...
          ...entries.map((e) => `  <url><loc>${baseUrl}${e.path}</loc></url>`),
          `</urlset>`,
        ].join("\n");
