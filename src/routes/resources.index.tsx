  ...
  head: () => ({
    meta: [
      { title: "Resources & Technical Guides | CNC Maintenance, Filtration | Alfa Tooling" },
      {
        name: "description",
        content:
          "Maintenance guides, buying guides and technical articles on CNC tooling, filtration and industrial spare parts.",
      },
      ...
      <section className="container mx-auto px-4 py-12 grid md:grid-cols-2 gap-6">
        {posts.map((p) => (
          <Link
            ...
            to="/resources/$slug"
            params={{ slug: p.slug }}
            className="border border-border rounded-lg p-6 bg-card hover:border-orange transition-colors block"
          >
