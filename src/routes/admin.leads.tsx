  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-3">
        {data?.length === 0 && (
          <p className="text-muted-foreground text-sm">No contact leads yet.</p>
        )}
        {data?.map((l) => (
          <div key={l.id} className="bg-background border border-border rounded-lg p-5">
            ...
            <div className="text-xs text-muted-foreground">
              {l.company} · {l.phone} · {new Date(l.created_at).toLocaleString()}
            </div>
            <div className="mt-3 text-sm bg-secondary rounded p-3 whitespace-pre-wrap">
              {l.message}
            </div>
          </div>
        ))}
