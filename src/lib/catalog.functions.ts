...
export const getProductBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const brand = product.brand_id
      ? toId(await brands.findOne({ _id: new ObjectId(product.brand_id) }))
      : null;
    const category = product.category_id
      ? toId(await categories.findOne({ _id: new ObjectId(product.category_id) }))
      : null;
    ...
    const related = product.category_id
      ? (
          await products
            .find({ category_id: product.category_id, _id: { $ne: product._id } })
            .limit(4)
            .toArray()
        ).map(toId)
      : [];
...
export const searchProducts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const prods = await products
      .find({
        is_published: true,
        $or: [{ name: regex }, { sku: regex }, { short_description: regex }],
      })
      .limit(30)
      .toArray();
